...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	speed := meanSpeed(action, duration)
	firstMultiplier := runningCaloriesMeanSpeedMultiplier * speed * runningCaloriesMeanSpeedShift
	return firstMultiplier * weight / mInKm * duration * minInH
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	heightInM := float64(height) / cmInM
	firstTerm := walkingCaloriesWeightMultiplier * weight
	secondTerm := (speed * speed / heightInM) * walkingSpeedHeightMultiplier * weight
	return (firstTerm + secondTerm) * duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	speed := swimmingMeanSpeed(lengthPool, countPool, duration)
	firstMultiplier := speed + swimmingCaloriesMeanSpeedShift
	return firstMultiplier * swimmingCaloriesWeightMultiplier * weight * duration
}
